...
String outgoingGatewayAddress = "3b17c99cc6";
...
//   try to parse packet
  ...
  //    Serial.println("Reject call cronometer start: ");
//    Serial.print(millis());

  onReceive(LoRa.parsePacket(), outgoingNetworkAddressFlag);
    Serial.println();
    Serial.print("Server call cronometer stop: ");
    Serial.print(millis());
    Serial.println();
}
  ...
  String sendFlag(){
    ...
    String outgoingSensorList;
    String flag = "";
    int outgoingSensorListContor;
...
//    http.begin("http://192.168.0.18:32332/api/gateways/" + outgoingGatewayAddress);  //Specify request destination
    ...
    if (httpCodeGET > 0) { //Check the returning code
      ...
      outgoingSensorListContor = sensors.size();
      numberOfDevices = String(outgoingSensorListContor);

      for(int i=0; i<outgoingSensorListContor; i++){
        
      const char* network = root["network"];
      const char* sensor = root["sensors"][i];

      outgoingNetworkAddress = String(network);
      outgoingSensorList = String(sensor);

      Serial.println();
      Serial.println("SERVER GET RESPONSE");
      Serial.println("Response: " + String(httpCodeGET));   //Print HTTP return code
      Serial.println("network: " + outgoingNetworkAddress);   
      Serial.println("sensors: " + outgoingSensorList);
      Serial.println();

      flag = outgoingNetworkAddress + "|" + outgoingGatewayAddress + "|" + outgoingSensorList;              // flag message
...
//    Serial.println("outgoingSensorList: " + outgoingSensorList);
    Serial.println();
    digitalWrite(LED_lora, LOW); 
    }
   }
...
//    //WIFI
  ...
  if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    ...
    JSONencoder["gatewayAddress"] = outgoingGatewayAddress;
    JSONencoder["sensorAddress"] = incomingSensorAddress;
