...
String localization(){
    String rssi = "0";
    String interogator = "1";
    String localizationFlag = "0";
    String outgoingNetworkAddress = "38465thdku";
    String outgoingSensorList[] = {"oftmdjrtgl", "4637281930", "94jfc84jg8", "ghdj475843", "jgfld75tn9", "01234abcde", "ggth44487h", "44444ggggg", "ff4gtdtgd3", "ffff8888r5"};
    String outgoingLocalization = "";
    int outgoingSensorListContor = sizeof(outgoingSensorList)/12;
    String rssiList[outgoingSensorListContor];
    String numberOfDevices = String(outgoingSensorListContor);
    if(outgoingSensorListContor == 0){
      return "0";  
    }
     for(int i=0; i<outgoingSensorListContor; i++){
     outgoingLocalization = outgoingNetworkAddress + "|" + outgoingGatewayAddress + "|" + outgoingSensorList[i];              // flag message
      if(interogator == "1"){
        LoRa.beginPacket();                  // start packet
        if(LoRa.beginPacket()){
          digitalWrite(LED_lora, HIGH);  
...
//      Serial.println("Snr: " + String(LoRa.packetSnr()));
      ...
      if(i <= outgoingSensorListContor){
        rssi = "0";
        while(rssi == "0"){
          rssi = receiveLocalization(LoRa.parsePacket(), outgoingNetworkAddress);
        }
        rssiList[i] = rssi;
        Serial.println();
        Serial.print("RSSI: ");
...
String receiveLocalization(int packetSize, String outgoingNetworkAddress){
