...
static unsigned long nextSwitchTimeLocalization = millis();
...
//    if(nextSwitchTimeLocalization<millis()){
      activateLocalizaton = localization();
//      nextSwitchTimeLocalization = millis()+60000;
//    }
      lastSendTimeFlag = millis();            // timestamp the message
      interval = random(2000) + 1000;    // 2-3 seconds
  }
     receiveLocalization(LoRa.parsePacket(), outgoingNetworkAddressFlag);

        
  if(activateLocalizaton == "0"){
    onReceive(LoRa.parsePacket(), outgoingNetworkAddressFlag);
  }
}

  String getLocalizationAddress() {
  ...
  String sendFlag(){
    String outgoingNetworkAddress = "n1e2t3w4o5";
    String outgoingSensorList[] = {"e1e2e3e4e5", "4637281930", "94jfc84jg8", "ghdj475843", "jgfld75tn9", "01234abcde", "ggth44487h", "44444ggggg", "ff4gtdtgd3", "ffff8888r5"};
    String flag = "";
...
String localization(){
    String outgoingNetworkAddress = "n1e2t3w4o5";
    String outgoingSensorLocalization = "e1e2e3e4e5";
    String outgoingLocalization = "";
    String interogator = "0";

    if(interogator == "0")
      return "1";
    
     outgoingLocalization = outgoingNetworkAddress + "|" + outgoingGatewayAddress + "|" + outgoingSensorLocalization;              // flag message
        ...
        LoRa.write(localAddress);             // add sender address
        LoRa.write(msgCount);                 // add message ID
        LoRa.write(outgoingLocalization.length());        // add payload length
        LoRa.print(outgoingLocalization);  // add payload
        LoRa.endPacket();                     // finish packet and send it
        msgCount++;                           // increment message ID 

        // SERIAL MONITOR
        Serial.println();
        Serial.println("LoRa GATEWAY SENDING localization");
//      Serial.println("destinationAddress: 0x" + String(destinationAddress, HEX));
//      Serial.println("localAddress: 0x" + String(localAddress, HEX));
//      Serial.println("msgCount: " + String(msgCount));
//      Serial.println("flag.length: " + String(flag.length()));
//      Serial.println("numberOfDevices: " + numberOfDevices);
//      Serial.println("outgoingLocalization: " + outgoingLocalization);
//      Serial.println("outgoingNetworkAddress: " + outgoingNetworkAddress);
//      Serial.println("outgoingGatewayAddress: " + outgoingGatewayAddress);
//      Serial.println("outgoingSensorList: " + outgoingSensorList[i]);
...
void receiveLocalization(int packetSize, String outgoingNetworkAddress){
  ...
  String incomingLocalization = "";
  String incomingNetworkAddress = "";
  String incomingGatewayAddress = "";
  String incomingSensorAddress = "";
  String incomingSensorId = "";
  
  while (LoRa.available()) {
    incomingLocalization += (char)LoRa.read();
  }

   incomingSensorAddress = incomingLocalization.substring(0, 10);
   incomingNetworkAddress = incomingLocalization.substring(11, 21);
   incomingGatewayAddress = incomingLocalization.substring(22, 32);
   incomingSensorId = incomingLocalization.substring(33, 37);
  
  if (incomingLength != incomingLocalization.length()) {   // check length for error
    Serial.println("Error: Message localization length does not match length.");
    digitalWrite(LED_lora, LOW);
  ...
  if (recipient != localAddress && recipient != 0xFF) {
    Serial.println("Error: This hexa localization address is not for me.");
    digitalWrite(LED_lora, LOW);
    return;                         // skip rest of function
  }
  ...
  double rssiDistance = pow(10, (-55-LoRa.packetRssi())/(10*3.0));
  ...
  if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    ...
    JSONencoder["address"] = outgoingGatewayAddress;
    JSONencoder["r"] = rssiDistance;
    JSONencoder["id"] = incomingSensorId;
    ...
    http.begin("http://192.168.0.27:7071/api/buffer");
...
//  Serial.println("incomingLocalization: " + incomingLocalization);
...
//  Serial.println("incomingSensorAddress: " + incomingSensorAddress);
  Serial.println("Gateway Address: " + outgoingGatewayAddress);
  Serial.println("RSSI: " + String(LoRa.packetRssi()));
  Serial.println("Snr: " + String(LoRa.packetSnr()));
  Serial.println("rssiDistance: " + String(rssiDistance));
  Serial.println();
